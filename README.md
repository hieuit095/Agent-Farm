...
Agent-Farm is an autonomous AI agent ecosystem designed to crawl GitHub, pinpoint real security vulnerabilities or code flaws, generate high-quality patches, validate fixes dynamically in isolated sandboxes, and submit pull requests or private disclosures. It implements advanced self-correcting DEV-QA loops to ensure every contribution is high-value, precise, and completely regression-free.
...
* **Pass 2 (Regression):** Executes the project's native test suite to ensure the patch does not break any existing functionality.

### 🤖 Terminator Mode Execution
A relentless, database-driven continuous operational loop (`superhuman`). It runs `hunt-circular` and `patrol` with no artificial delays, fully utilizing token pool rotation and safely adhering to daily quota caps.

---

## 🛠️ System Architecture (High-Level)

1. **Target Discovery & Orchestration:** Identifies targets manually via CLI or autonomously using round-robin JSON databases.
2. **Analysis & RAG Processing:** Clones repos, scans with white-hat Semgrep rules, maps AST dependencies, and indexes documentation into ChromaDB.
3. **Generation & DEV-QA Sandbox Loop:** LLMs write PoCs and patches, which are rigidly tested against native tests in isolated Docker containers (`docker>=7.1`). Errors trigger a self-correction retry loop.
4. **Supreme Audit & PR:** Verified patches pass a secondary audit (Gemini 3.5 Flash) before either generating a PR or logging private security disclosures.

---

## 📦 Getting Started (1-Click Docker Quick-Start)
...
* `OPENROUTER_API_KEY`: Key for OpenRouter integration.
* `MINIMAX_API_KEY`: API Key for Minimax integrations (if used).
...
Agent-Farm provides a comprehensive suite of Click-based CLI utilities. Ensure you are in the Docker container or a properly configured environment.

```bash
# Attach to the Agent CLI inside docker:
docker exec -it agent-farm bash

# Run the relentless Terminator loop (patrols PRs and hunts targets continuously)
