...
* **Dynamic Bug Verification**: Before writing a fix, the agent generates a self-contained Proof-of-Concept (PoC) script using `deepseek-v4-pro` to dynamically trigger the vulnerability inside a locked-down container sandbox. If the PoC fails to trigger the bug, the finding is immediately classified as a False Positive and dropped.
...
Agent-Farm's architecture consists of multiple pipelines working together. When a repository is targeted, it undergoes:
1. **Early Initialization & Baseline Setup**: Clones the repo locally and establishes a baseline by running native tests.
2. **Contextual Discovery (Omniscient Context Engine)**: Recursively parses project guidelines and architecture docs, indexing them in ChromaDB. AST maps dependencies across source files.
3. **Multi-layer Analysis & Gate Checks**: Code is analyzed by LLMs. Findings must pass strict constraints, including the **Anti-Farming Filter**, to confirm they are genuine and impactful.
4. **Dynamic Fix & QA Loops**: If an issue is verified, the system generates a PoC in an isolated Sandbox, drafts a fix, and runs double-pass validation to guarantee the exploit is resolved without breaking tests.
5. **Final Review & Dispatch**: Passes final supreme audit before either creating a Pull Request or initiating a private security disclosure (via private webhook/alerts).
...
3. **Configure Environment Variables:**
   Open the newly created `.env` file and configure the required settings:
   ```env
   ...
   GITHUB_SECONDARY_TOKENS=optional_secondary_tokens
   ```
...
*(Note: The legacy `janitor` command has been deprecated and disabled.)*
