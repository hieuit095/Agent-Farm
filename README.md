...
Agent-Farm operates as a multi-stage pipeline:
1. **Target Discovery:** Discovers repositories matching criteria, filtering out repositories restricted to prior contributors or with hostile maintainer vibes.
2. **Analysis & Indexing:** Clones target, discovers subsystem docs, chunks and indexes into ChromaDB (Omniscient Context Engine). Generates AST dependency graphs.
3. **Vulnerability Analysis:** Employs Bloodhound Red Team audits via Semgrep and deep code scans to find security issues or code improvements. Passes findings through the Anti-Farming Filter.
4. **Verification & Generation:** Uses a dynamic PoC to prove vulnerabilities. Generates patches with `deepseek-v4-pro` and passes them through an internal DEV-QA loop and Layer 2 Supreme Audit (`gemini-3.5-flash`).
5. **Execution & PR Creation:** Verifies the patch via Docker sandbox (Blast Radius check) and creates a PR or private security disclosure.
...
3. **Configure Environment Variables:**
   Open the newly created `.env` file and configure your API tokens (core variables):
   ```env
   GITHUB_TOKEN=your_github_pat_here
   OPENROUTER_API_KEY=your_openrouter_key_here
   TELEGRAM_BOT_TOKEN=your_telegram_token_here  # Optional: For notifications
   ```
