...
async def test_omni_e2e_pipeline(tmp_path):
    ...
    with contextlib.ExitStack() as stack:
        ...
        if pipeline._llm:
            pipeline._llm.close = AsyncMock()  # Fix mock await error
        else:
            pipeline._llm = MagicMock()
            pipeline._llm.close = AsyncMock()

        # Let's completely patch the client of OpenRouterProvider since we replaced it with Mock
        with patch.object(OpenRouterProvider, "close", new_callable=AsyncMock):
