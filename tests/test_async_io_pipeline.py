...
class TestAsyncIOPipeline(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_clone_and_patch_repo_uses_to_thread(self, mock_gettempdir, mock_join, mock_to_thread):
        ...
        with patch("farm_agent.orchestrator.pipeline.asyncio.gather", new_callable=unittest.mock.AsyncMock, side_effect=lambda *args, **kwargs: __import__("asyncio").gather(*args, **kwargs)) as mock_gather:
