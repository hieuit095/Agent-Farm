...
async def get_logs_today(lines: int = 100):
    """Get the last N lines of today's log file.

    Returns a JSON object with the log lines and metadata.
    Safe: returns empty array if no log file exists yet.
    """
    from contribai.core.logger import get_today_log_path

    log_dir = _config.logging.log_dir if _config else "logs"
    log_path = get_today_log_path(log_dir)

    if log_path is None:
        return {"lines": [], "message": "No log file for today yet.", "file": None}
    ...
    try:
        ...
        return {
            "lines": [],
            "message": "Cannot read log file: permission denied.",
            "file": str(log_path),
        }
