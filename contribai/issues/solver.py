...
"""
    ...
    async def solve_issue_deep(
        ...
        # Phase 2.5: Generate project skeleton map for LLM navigation
        mapper = RepoMapper()
        ...
        try:
            skeleton_map = await mapper.generate_map(context.file_tree, _fetch_content)
        except Exception as exc:
            logger.debug("RepoMapper failed for %s: %s", repo.full_name, exc)
            skeleton_map = ""
        ...
        if skeleton_map:
            skeleton_section = (
                f"## Project Map (structural skeleton — signatures only):\n"
                f"```\n{skeleton_map[:60000]}\n```\n\n"
            )
...
## Instructions:
1. Based on the Issue description and the provided Project Map, list the
   exact file paths you need to read in full to fix this issue.
