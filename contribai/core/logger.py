"""Daily rotating file logger for ContribAI.

Provides a TimedRotatingFileHandler that creates a new log file
at midnight each day. Log files are named `contribai_YYYY-MM-DD.log`
...
from contribai.core.config import LogConfig
...
def setup_daily_logger(config: LogConfig) -> logging.Logger:
    ...
    Creates ``logs/contribai_YYYY-MM-DD.log`` files that rotate at
    ...
    # Build the log file path: logs/contribai_YYYY-MM-DD.log
    today = datetime.now(UTC).strftime("%Y-%m-%d")
    log_file = log_dir / f"contribai_{today}.log"
...
def get_today_log_path(log_dir: str = "logs") -> Path | None:
    ...
    today = datetime.now(UTC).strftime("%Y-%m-%d")
    log_file = Path(log_dir) / f"contribai_{today}.log"
