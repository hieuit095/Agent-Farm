...
class DockerSandbox:
    ...
    async def run_in_sandbox(
        ...
        container_name = f"contribai-sandbox-{run_id[:12]}"
        labels = {
            "contribai.sandbox": "true",
            "contribai.sandbox.run_id": run_id,
            "contribai.sandbox.repo_path": str(repo_dir),
        }
    ...
    def _list_run_containers(self, run_id: str) -> list[Container]:
        ...
        return self.client.containers.list(
            all=True,
            filters={"label": [f"contribai.sandbox.run_id={run_id}"]},
        )
