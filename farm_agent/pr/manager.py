    ...
    async def create_pr(
        ...
        *,
        ...
        try:
            ...
            # ── Gag Order — block AI disclosures in PR body before submission ──
            import re as _re
            _GHOST_RE = _re.compile(
                r"(?i)(as an ai|ai generated|language model|openai|minimax|"
                r"automated bot|generated by ai|ai assistant|contribai|"
                r"this pr was generated)",
            )
            if _GHOST_RE.search(pr_body):
                logger.warning(
                    "Gag Order triggered — AI disclosure in PR body for %s. Aborting PR.",
                    contribution.title,
                )
                raise ValueError(
                    f"PR body contains forbidden AI disclosure: {contribution.title}"
                )
            # ─────────────────────────────────────────────────────────────────────
