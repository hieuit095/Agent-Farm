...
def auto_check_pr_template(body: str, contrib_type: ContributionType | None = None) -> str:
    ...
    for i, line in enumerate(lines):
        ...
        if (
            ...
            and not any(
                danger in stripped
                for danger in ["breaking", "release", "deploy", "migration"]
            )
        ):
    ...
    async def create_pr(
        ...
        *,
        ...
        try:
            ...
            author_id = user.get('id', '9919')
            author_login = user.get('login', 'farm_agent')
            author_email = user.get("email") or f"{author_id}+{author_login}@noreply.github.com"
            past_date = datetime.now(UTC) - timedelta(minutes=random.randint(15, 45))
            author_date = past_date.strftime("%Y-%m-%dT%H:%M:%SZ")
