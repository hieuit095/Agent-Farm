...
{"decision": "REJECT", "critique": "Variable `foo` is used but never defined in scope."}
"""
...
"""
    ...
    def _parse_verdict(self, response: str) -> dict:
        ...
        try:
            ...
            return {"decision": "REJECT", "critique": f"[Fail-Closed — JSON parse failed, raw: {response[:200]}]"}  # noqa: E501

        decision = parsed.get("decision", "REJECT").strip().upper()
        if decision not in ("APPROVE", "REJECT"):
            logger.warning("ReviewerAgent: unknown decision '%s' — defaulting to REJECT (Fail-Closed)", decision)  # noqa: E501
            decision = "REJECT"
