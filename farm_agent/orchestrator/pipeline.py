    ...
    async def _validate_findings(
        ...
        for finding in findings:
            ...
            try:
                response = await self._llm.complete(
                    ...
                    system=(
                        ...
                        "vulnerable sink. If no clear exploitable data flow exists, mark as "
                        "False Positive.\n"
                        "3. GARBAGE SNIPPET REJECTION: If snippet is too short, is just a "
                        "string literal, or lacks structural programming context, "
                        "reject immediately.\n\n"
                        "KNOWN FALSE POSITIVES IMMUNITY LIST:\n"
                        "- GO LANG: defer guarantees execution. defer mutex.Unlock() is safe "
                        "and the OPPOSITE of a deadlock. NEVER flag it.\n"
                        ...
                        "- ALL: If the snippet is NOT valid programming code (e.g., just "
                        "English text like 'TLS configuration missing'), DROP IT.\n\n"
                        ...
                        '"data_flow_proof": "string (required if true — exact var names)"}'
                    ),
                    temperature=0.1,
                )
            except Exception as e:
            ...
            try:
                ...
                _h_words = {" If ", " Assume ", " Might ", " Maybe ", " Possibly ", " Probably "}
                if is_real and (
                    len(data_flow_proof) < 20
                    or data_flow_proof.lower().count("if") > 2
                    or any(w in data_flow_proof for w in _h_words)
                ):
                    logger.info(
