...
class FarmAgentPipeline:
    ...
    def __init__(self, config: FarmAgentConfig):
        ...
        if (
            getattr(self.config.pipeline.max_concurrent_repos, "__gt__", None)
            and getattr(_safe_cap, "__lt__", None)
            and hasattr(self.config.pipeline.max_concurrent_repos, "__int__")
            and hasattr(_safe_cap, "__int__")
            and int(self.config.pipeline.max_concurrent_repos) > int(_safe_cap)
        ):
            logger.info(
