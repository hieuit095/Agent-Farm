    ...
    async def _validate_findings(
        ...
        for finding in findings:
            ...
            prompt = (
                ...
                '{"devil_advocate_critique": "MANDATORY: Write 2 sentences explaining why this '
                'snippet is perfectly safe, normal, or uses modern language defaults. '
                'Prove the scanner wrong.", '
                '"is_real_vulnerability": true/false, "confidence_score": 0-100, '
                '"rejection_reason": "reason if false", '
                '"data_flow_proof": "exact var names if true"}'
            )
            ...
            try:
                response = await self._llm.complete(
                    ...
                    system=(
                        ...
                        "code snippet. Do NOT assume, guess, or imagine functionality not visible. "
                        "Do NOT use 'If [condition]' logic. If you have to say 'If', it is a "
                        "False Positive.\n"
                        "2. DATA FLOW REQUIREMENT: You must trace user-controlled input to the "
                        "vulnerable sink. If no clear exploitable data flow exists, mark as False "
                        "Positive.\n"
                        ...
                        "- PYTHON: Standard urllib or requests usages WITHOUT explicit unsanitized "
                        "user inputs in the URL are safe.\n"
                        "- ALL: If the snippet is NOT valid programming code (e.g., just English "
                        "text like 'TLS configuration missing'), DROP IT.\n\n"
            ...
            try:
                ...
                if not is_real or confidence < 90:
                    da_trunc = (
                        (devil_advocate[:60] + "...")
                        if len(devil_advocate) > 60 else devil_advocate
                    )
                    logger.info(
                ...
                df_trunc = (
                    (data_flow_proof[:60] + "...")
                    if len(data_flow_proof) > 60 else data_flow_proof
                )
                logger.info(
