...
> **M2 phase 6 (2026-09-25):** Proof integrity and automatic handoff. [security/oracles.py](file:///C:/Users/USER/Documents/GitHub/Bug-Bounty/Agent-Farm/farm_agent/security/oracles.py) now emits a retrievable, redacted proof record (ordered `benign_before → malicious_before → malicious_after → benign_after` with method/url/role/impact/status/transport_error/body_sha256/body_len/witness_hits, plus oracle digest, surface, risk class, tenants, object id and target commit); raw bodies and header values are never stored. [orchestrator/memory.py](file:///C:/Users/USER/Documents/GitHub/Bug-Bounty/Agent-Farm/farm_agent/orchestrator/memory.py) adds a `security_proofs` table (`record_hash`, `valid`, `vulnerability_confirmed`, `patch_status`) with `store_security_proof`/`get_security_proof`/`list_security_proofs`; a non-confirming run invalidates prior valid proofs for that candidate, so contradictory or stale evidence cannot authorize publication, and `security_candidate_has_semantic_proof` now reads validated proofs. [security/verifier.py](file:///C:/Users/USER/Documents/GitHub/Bug-Bounty/Agent-Farm/farm_agent/security/verifier.py) keeps vulnerability confirmation separate from patch status, persists the proof before closure, and accepts `NEEDS_MANUAL_REVIEW`. [orchestrator/pipeline.py](file:///C:/Users/USER/Documents/GitHub/Bug-Bounty/Agent-Farm/farm_agent/orchestrator/pipeline.py) no longer marks a PoC-confirmed candidate `CONFIRMED`: it lands in `NEEDS_MANUAL_REVIEW` (`SEMANTIC_PROOF_PENDING`) so automatic publication requires the operator's semantic verifier (both `run_circular` and standard paths route through `_process_repo`). [security/artifacts.py](file:///C:/Users/USER/Documents/GitHub/Bug-Bounty/Agent-Farm/farm_agent/security/artifacts.py) reports a missing canonical artifact as a gate error. Verified: focused M2 proof suite 33 passed; full suite 142 passed with the pre-existing AsyncMock warning; Docker artifact 2 passed. Replay is by re-running the canonical oracle; bodies are stored as digests, not raw. **M2 proof/integration criteria hold on the tested paths; M3 may proceed.**

> **M2 proof-integrity gate (2026-09-25):** Publication reads now validate the whole chain instead of trusting a proof row. [orchestrator/memory.py](file:///C:/Users/USER/Documents/GitHub/Bug-Bounty/Agent-Farm/farm_agent/orchestrator/memory.py) `security_candidate_has_semantic_proof` checks the canonical hash (`sha256(record_json) == record_hash`), the record fields against the stored columns (oracle digest, surface, risk class, commit), the candidate/scan/repo/commit, the closing `SEMANTIC_PROOF` evidence `content_hash == proof.evidence_hash`, the live manifest scope and the threat model commit; any mismatch or unparseable record denies publication (schema migration adds `after_endpoint` and `evidence_hash`). [security/oracles.py](file:///C:/Users/USER/Documents/GitHub/Bug-Bounty/Agent-Farm/farm_agent/security/oracles.py) `_redacted_url` keeps scheme/host/port/path and query keys and drops userinfo, query values (`REDACTED`) and fragments, so secret-query probe URLs are not persisted while the protected canonical artifact retains the full URL for an authorized replay. `ProofResult.after_endpoint` (blocked/exposed/regressed/inconclusive) is stored separately from `patch_status`, which stays `unverified` in M2 because a patch verdict needs the M5 fix contract. Both automatic entry points (`_process_repo`, `run_circular`) close a PoC-confirmed candidate as `NEEDS_MANUAL_REVIEW`/`SEMANTIC_PROOF_PENDING` and cannot publish until the operator's `SemanticVerifier` adds a valid proof. Verified: focused M2 integration 47 passed; full suite 155 passed with the pre-existing AsyncMock warning; Docker artifact 2 passed. **M2 PASS — proceed to Prompt 3.**

> **M3 slice 1 (2026-09-25):** Removed the two gates that lost distinct same-repository candidates in [pipeline.py](file:///C:/Users/USER/Documents/GitHub/Bug-Bounty/Agent-Farm/farm_agent/orchestrator/pipeline.py) `_process_repo_impl` — the per-repo `[:2]` investigation cap and the file-only dedupe against past PRs. Investigation is now bounded by a separate `pipeline.max_candidates_investigated` (default 25, [config.py](file:///C:/Users/USER/Documents/GitHub/Bug-Bounty/Agent-Farm/farm_agent/core/config.py)); the publication cap (`max_prs`) and the title-similarity duplicate check against published PRs are retained. Verified: three distinct findings in one file with a past PR on that file now yield three `security_candidates` and a `dedupe`/`survived` count of 3 in `scan_events`; an investigation limit of 2 yields two candidates while `max_prs=1` (`INVESTIGATION_LIMIT`). Focused M3 test 2 passed; full suite 157 passed with the pre-existing AsyncMock warning; Docker artifact 2 passed. **M3 sub-slices 2–4 (root-cause identity and multi-sensor evidence merge, route/authz/cross-file lanes, controlled recall benchmark) remain open; the M3 gate is not met and the ≥98% production precision target is unclaimed.**

> **M3 slice 2 (2026-09-25):** Durable admission and root-cause identity. [pipeline.py](file:///C:/Users/USER/Documents/GitHub/Bug-Bounty/Agent-Farm/farm_agent/orchestrator/pipeline.py) no longer truncates detection: every distinct finding is admitted as a `security_candidates` row at the pinned SHA, active investigation is bounded by `pipeline.max_candidates_investigated`, and surplus work is parked `DEFERRED` (`INVESTIGATION_DEFERRED`) with an M0 coverage event and resumes on the next run; relevant-file fetching is bounded by the same limit. [security/identity.py](file:///C:/Users/USER/Documents/GitHub/Bug-Bounty/Agent-Farm/farm_agent/security/identity.py) (new) derives a commit-pinned root-cause identity from actor/resource/security boundary/source/control/sink with title and severity excluded (title folded in only when no structural component is known, so unknowns never merge unrelated findings). [orchestrator/memory.py](file:///C:/Users/USER/Documents/GitHub/Bug-Bounty/Agent-Farm/farm_agent/orchestrator/memory.py) adds `root_cause_fingerprint`, `defer_reason` and `security_evidence.origin`, a partial unique index on `(repo, target_commit, root_cause_fingerprint)`, idempotent `create_security_candidate`, `defer`/`mark_candidate_investigating`, and `list_candidate_fingerprints`/`list_pending_security_candidates`. Verified with real SQLite: 30 findings → 30 candidates (5 active, 25 deferred), restart resumes to 0 deferred with no duplicates; equal titles+severity at different sinks stay 2 candidates; two sensors merge into 1 candidate with both evidence rows; two concurrent workers create 1 candidate; a candidate cannot borrow another's proof. Focused M3 tests 7 passed; full suite 162 passed with the pre-existing AsyncMock warning; Docker artifact 2 passed. **M3 sub-slice 3 (route/authz/cross-file/SCA lanes) and sub-slice 4 (controlled recall benchmark) remain open; the M3 gate is not met and the ≥98% production precision target is unclaimed.**

> **M3 slice 3 (2026-09-25):** No silent erasure, exact resume input, identity hardening. [pipeline.py](file:///C:/Users/USER/Documents/GitHub/Bug-Bounty/Agent-Farm/farm_agent/orchestrator/pipeline.py) records every filtered `SECURITY_FIX` finding as a durable candidate closed `OPEN_PROOF_GAP` with a stage reason (`FILTERED_NON_CODE_TARGET`, `FILTERED_LOW_IMPACT`, `DUPLICATE_PUBLISHED_TITLE`, `VALIDATION_REJECTED`, `APPRAISAL_REJECTED`) plus an M0 event; publication limits stay separate from admission. `security_candidates.investigation_json` persists the minimum investigation input (type, severity, title, description, file_path, line_start, sanitized metadata, sensor) bound to repo/scan/SHA, and resume loads that exact input; rows without it are deferred `INVESTIGATION_INPUT_MISSING` rather than reconstructed or defaulted to MEDIUM. [security/identity.py](file:///C:/Users/USER/Documents/GitHub/Bug-Bounty/Agent-Farm/farm_agent/security/identity.py) keeps the same file/line/title/severity with different actor/resource/sink distinct, and falls back to the title when no structural component is known. The pipeline groups same-fingerprint findings into one candidate and records a `SENSOR_OBSERVATION` evidence row per contributing sensor, so two sensors for one root cause yield one candidate with both evidence records; two workers create one candidate, both evidence rows persist, and exactly one close wins. Verified: focused M3 tests 15 passed; full suite 169 passed with the pre-existing AsyncMock warning; Docker artifact 2 passed; changed-line Ruff 0 new findings; repo total 687. **M3 sub-slice 4 (route/API inventory, authz matrix, cross-file source-to-sink, Semgrep-lane isolation, secret/SCA/IaC) and sub-slice 5 (controlled recall benchmark) remain open; the M3 gate is not met and the ≥98% production precision target is unclaimed.**
