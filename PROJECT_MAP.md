...
| Config | Pydantic v2 + YAML + `.env` | `config.py` |
...
├── docker-compose.yml                  # agent-farm service definition with volumes
...
│   │
│   ├── plugins/
│   │   └── __init__.py                 # Plugins submodule
│   │
│   ├── notifications/
│   │   └── notifier.py                 # Notifications submodule
│   │
│   ├── pr/
...
### 3A. Standard Pipeline — `_process_repo()`
...
_process_repo(repo)
  ...
  │    ├─ Gate 4a: Drops security findings unless severity is CRITICAL or HIGH
...
### 3B. Circular target pipeline — `run_circular()`
...
The `PRPatrol` module runs continuously to manage active contributions:
...
The CLI is Click-based and located in `farm_agent/cli/main.py`.
...
| `farm_agent superhuman` | `SuperHumanLoop.run_daily_routine()` | Terminator Mode relentless loop cycling through circular target hunt and patrol operations. |
