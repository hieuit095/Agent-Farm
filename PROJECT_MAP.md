...
```mermaid
graph TD
    CLI["CLI (main.py)"] --> Pipeline["Orchestrator Pipeline (pipeline.py)"]
    CLI --> Patrol["PR Patrol (patrol.py)"]
    CLI --> HumanLoop["SuperHumanLoop (human.py)"]

    Pipeline --> GitHub["GitHubClient (client.py)"]
    Pipeline --> Analyzer["CodeAnalyzer (analyzer.py)"]
    Pipeline --> Appraiser["Layer 1 Expert Appraisal (Qwen)"]
    Pipeline --> GenPoC["PoC Generator (poc.py)"]
    Pipeline --> Generator["ContributionGenerator (engine.py)"]
    Pipeline --> Validator["Layer 2 Supreme Audit (Gemini)"]
    Pipeline --> PRManager["PRManager (manager.py)"]

    Generator --> Sandbox["DockerSandbox (sandbox.py)"]
    GenPoC --> Sandbox

    Analyzer --> RAG["RAG Engine (rag.py)"]
    Analyzer --> Mapper["RepoMapper (mapper.py)"]

    Pipeline -.-> Memory["Memory SQLite (memory.py)"]
    Patrol -.-> Memory
```
...
### 5B. Circular target pipeline — `run_circular()` ([pipeline.py:834](file:///c:/Users/USER/Documents/GitHub/Agent-Farm/farm_agent/orchestrator/pipeline.py#L834))
